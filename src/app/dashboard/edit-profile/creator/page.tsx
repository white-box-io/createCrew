...
import { ArrowLeft, Save, Megaphone, Check } from "lucide-react";
...
import CreatorForm from "@/components/edit-profile/creator-form";
...
export default function EditCreatorProfilePage() {
  ...
  if (isLoading || !user || !profile || !user.creatorMode) {
    return (
    ...
    <section className="min-h-[calc(100vh-4rem)] bg-brand-surface pb-28 pt-8">
      <div className="mx-auto max-w-2xl px-5 lg:px-8">
        ...
        >
          ...
          <h1 className="text-xl font-bold text-brand-dark sm:text-2xl">Edit Creator Profile</h1>
          <p className="mt-1 text-sm text-brand-gray">
            Tell freelancers about your channel, audience, and the kind of crew you're looking for! 🎬
          </p>
        </motion.div>
        ...
        >
          <div className="mb-6 flex items-center gap-2 border-b border-border/40 pb-4">
            <Megaphone className="h-5 w-5 text-brand-coral" />
            <h2 className="text-lg font-bold text-brand-coral">Creator Information</h2>
          </div>
          
          <CreatorForm
            data={profile.creator}
            onChange={(newData: CreatorProfile) =>
              setProfile({ ...profile, creator: newData })
            }
          />
        </motion.div>
      </div>
        ...
        <div className="mx-auto flex max-w-2xl items-center justify-between px-1">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handleSave}
              disabled={saving}
              className="flex items-center gap-1.5 rounded-md bg-brand-coral px-5 py-2.5 text-sm font-semibold text-white transition-all hover:bg-brand-coral/90 disabled:opacity-70"
            >
